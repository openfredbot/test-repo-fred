...
/**
 * @title FredStaking
 * @dev Flexible staking contract for $FRED token
 * - No lock period - stake/unstake anytime
 * - 30% APY rewards calculated per second
 * - Rewards from pre-deposited pool
 */
contract FredStaking is ReentrancyGuard, Ownable {
    using SafeERC20 for IERC20;

    IERC20 public immutable stakingToken;
    
    // APY: 30% = 30e16 (in 18 decimals, divide by 1e18)
    uint256 public constant APY = 30e16; // 30%
    ...
    uint256 public totalRewardsPaid;  // Track total rewards distributed
    ...
    function pendingRewards(address _user) public view returns (uint256) {
        ...
        // rewards = (amount * APY * timeElapsed) / (SECONDS_PER_YEAR * 1e18)
        uint256 rewards = (stake.amount * APY * timeElapsed) / (SECONDS_PER_YEAR * 1e18);
    ...
    function _claimRewards(address _user) internal {
        ...
        if (rewards > 0) {
            ...
            if (rewards > 0) {
                ...
                stakes[_user].lastUpdateTime = block.timestamp;
                
                stakingToken.safeTransfer(_user, rewards);
    ...
    /**
     * @dev Deposit rewards into the pool (anyone can deposit)
     */
    ...
    function withdrawRewards(uint256 _amount) external onlyOwner {
        require(_amount <= rewardPool, "Insufficient reward pool");
        rewardPool -= _amount;
        stakingToken.safeTransfer(msg.sender, _amount);
    ...
    function getContractStats() external view returns (
        ...
        return (totalStaked, rewardPool, totalRewardsPaid, APY);
    }
}
